...
const POST_TYPE_CONFIG: Record<PostType, { label: string; color: string; bgColor: string; borderColor: string; icon: typeof Flame }> = {
  ...
  'question': { label: 'Question', color: 'text-[#D4A853]', bgColor: 'bg-[#D4A853]/10', borderColor: 'border-[#D4A853]/20', icon: MessageSquare },
};
          ...
          >
            <div className="flex items-start gap-3">
              <Link href={profileHref} className="flex-shrink-0">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#D4A853] to-[#B8922F] flex items-center justify-center text-white text-[10px] font-bold overflow-hidden ring-2 ring-[#0c0c10]">
            ...
            <AnimatePresence>
              {replyTo === comment.id && (
                ...
                >
                  ...
                  <Button size="sm" onClick={() => handleReply(comment.id)} disabled={!replyContent.trim()} className="bg-[#D4A853] hover:bg-[#B8922F] text-white h-8 w-8 p-0"><Send className="w-3.5 h-3.5" strokeWidth={1.5} /></Button>
                </motion.div>
              )}
            </AnimatePresence>
...
export default function PostCard({ post, communityId, onLikeToggle, onCommentToggle, index = 0 }: PostCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -inset-[1px] rounded-xl bg-gradient-to-r from-[#D4A853]/0 via-[#D4A853]/0 to-[#D4A853]/0 group-hover:from-[#D4A853]/20 group-hover:via-[#D4A853]/10 group-hover:to-[#D4A853]/20 transition-all duration-500 blur-sm" />
        ...
        <div className="p-4 sm:p-5 pb-0">
          <div className="flex items-start gap-3">
            ...
            <Link href={profileHref} className="flex-shrink-0 relative">
              <div className={`w-10 h-10 rounded-full bg-gradient-to-br from-[#D4A853] to-[#B8922F] flex items-center justify-center text-white text-xs font-bold overflow-hidden ring-2 ring-[#0c0c10] group-hover:ring-[#D4A853]/30 transition-all duration-300`}>
                {post.authorAvatar ? (
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <motion.div whileTap={{ scale: 0.9 }}>
                        <Button size="sm" onClick={handleComment} disabled={!commentInput.trim()} className="bg-[#D4A853] hover:bg-[#B8922F] text-white h-9 w-9 p-0 flex-shrink-0">
