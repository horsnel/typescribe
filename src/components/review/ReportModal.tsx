...
export default function ReportModal({ isOpen, onClose, onSubmit, contentType, contentPreview }: ReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-[#0c0c10] border border-[#1e1e28] rounded-2xl max-w-lg w-full max-h-[85vh] overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-[#1e1e28]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#6b7280]">Help us maintain a respectful community</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        {contentPreview && (
          <div className="px-5 pt-4">
            <div className="bg-[#050507] border border-[#1e1e28] rounded-lg p-3">
              <p className="text-xs text-[#6b7280] mb-1">Reported content:</p>
              <p className="text-sm text-[#9ca3af] line-clamp-2">{contentPreview}</p>
            </div>
          </div>
        )}
        ...
        {submitted ? (
          ...
          <>
                  ...
                  {REPORT_REASONS.map((reason) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className={`text-sm font-medium ${selectedReason === reason.value ? 'text-white' : 'text-[#9ca3af]'}`}>
                            {reason.label}
                          </span>
                          <p className="text-xs text-[#6b7280] mt-0.5">{reason.description}</p>
                        </div>
                      </div>
                    </button>
                  ))}
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-3 p-5 border-t border-[#1e1e28]">
              <Button
                variant="outline"
                onClick={handleClose}
