  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-6">
        ...
        <div className="flex items-center gap-1">
          {Array.from({ length: 10 }, (_, i) => (
            <button key={i + 1} type="button" onClick={() => { setRating(i + 1); setErrors((prev) => ({ ...prev, rating: undefined })); }} onMouseEnter={() => setHoverRating(i + 1)} onMouseLeave={() => setHoverRating(0)} className="p-0.5 transition-transform hover:scale-110">
              <Star className={`w-6 h-6 transition-colors ${i + 1 <= displayRating ? 'text-[#D4A853] fill-[#D4A853]' : 'text-[#2a2a35]'}`} strokeWidth={1.5} />
            </button>
          ))}
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" disabled={!isValid} className="bg-[#D4A853] hover:bg-[#B8922F] text-white gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><Send className="w-4 h-4" strokeWidth={1.5} />Submit Review</Button>
