...
export default function ReviewCard({
  ...
  if (variant === 'compact') {
    return (
      <div className="bg-[#0a0a0f] border border-[#2a2a35] rounded-xl p-4 flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {showMovieTitle && movieSlug && <Link href={`/movie/${movieSlug}`} className="text-xs text-[#6b6b7b] hover:text-[#a0a0b0] mb-1 block">Review of {movieSlug}</Link>}
          <p className="text-sm text-[#a0a0b0] line-clamp-2">{review.text}</p>
          <div className="flex items-center gap-4 mt-2">
            <button onClick={handleHelpful} className={cn('flex items-center gap-1 text-xs transition-colors', helped ? 'text-[#e50914]' : 'text-[#6b6b7b] hover:text-[#a0a0b0]')}><ThumbsUp className="w-3 h-3" /><span>{helpfulCount}</span></button>
            <span className="text-xs text-[#6b6b7b]">{formatDate(review.created_at)}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
      <div className={`bg-[#0a0a0f] border rounded-xl p-6 ${badge ? 'border-yellow-500/30' : 'border-[#2a2a35]'}`}>
        {/* Moderation Badge */}
        {badge && (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {showMovieTitle && movieSlug && (
              <p className="text-sm text-[#6b6b7b] mb-2">
                Review of{' '}
                <Link href={`/movie/${movieSlug}`} className="text-[#a0a0b0] font-medium hover:text-white transition-colors">
                  {movieSlug.replace(/-/g, ' ')}
                </Link>
              </p>
            )}
            ...
            <div className="flex items-center gap-6">
              <button onClick={handleHelpful} className={cn('flex items-center gap-1.5 text-xs transition-colors', helped ? 'text-[#e50914]' : 'text-[#6b6b7b] hover:text-[#a0a0b0]')}>
              ...
              {!isOwnReview && reported && (
                <span className="flex items-center gap-1.5 text-xs text-[#6b6b7b]">
              ...
              {isOwnReview && (
                <>
                  {onEdit && (
                    <button onClick={() => onEdit(review.id)} className="flex items-center gap-1.5 text-xs text-[#6b6b7b] hover:text-[#a0a0b0] transition-colors">
