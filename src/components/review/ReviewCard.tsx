  ...
  if (variant === 'compact') {
    return (
      <div className="bg-[#050507] border border-[#1e1e28] rounded-xl p-4 flex gap-3">
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#D4A853] to-[#B8922F] flex items-center justify-center text-white text-xs font-bold flex-shrink-0 overflow-hidden">
    ...
    <>
      <div className={`bg-[#050507] border rounded-xl p-6 ${badge ? 'border-yellow-500/30' : 'border-[#1e1e28]'}`}>
        ...
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#D4A853] to-[#B8922F] flex items-center justify-center text-white text-sm font-bold flex-shrink-0 overflow-hidden">
            {review.user_avatar ? <img src={review.user_avatar} alt={review.user_name} className="w-full h-full object-cover" /> : avatarFallback}
          </div>
