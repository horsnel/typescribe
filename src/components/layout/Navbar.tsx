...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { label: 'Games', href: '/games', isNew: true as const },
    { label: 'Discover', href: '/discover', isNew: true as const },
    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-12 transition-all duration-300 ${
        ...
        <div className="flex items-center gap-2 z-10">
          <button onClick={() => setSearchOpen(true)} className="hidden sm:flex items-center gap-2 px-4 py-2 bg-[#0c0c10] border border-[#1e1e28] rounded-full text-[#6b7280] hover:text-[#9ca3af] hover:border-[#3a3a45] focus:border-[#D4A853] transition-all text-sm" aria-label="Search">
          ...
          <button onClick={() => setSearchOpen(true)} className="sm:hidden p-2 text-[#9ca3af] hover:text-[#D4A853] transition-colors" aria-label="Search">
          ...
          {isAuthenticated ? (
            <div className="relative" ref={profileMenuRef}>
              <button
                ...
                aria-haspopup="true"
              >
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#D4A853] to-[#B8922F] flex items-center justify-center text-white text-xs font-bold overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/signup" className="hidden sm:block">
                <Button className="bg-[#D4A853] hover:bg-[#B8922F] text-white text-sm font-medium">Sign Up</Button>
              </Link>
            </div>
          )}

          <button className="md:hidden p-2 text-[#9ca3af] hover:text-[#D4A853] transition-colors" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Menu">
            ...
            <div className="border-t border-[#1e1e28] pt-4 mt-2">
              {isAuthenticated ? (
                ...
                <div className="flex flex-col gap-3">
                  <Link href="/login" onClick={() => setMobileMenuOpen(false)}><Button className="w-full bg-[#D4A853] hover:bg-[#B8922F] text-white font-medium">Sign In</Button></Link>
