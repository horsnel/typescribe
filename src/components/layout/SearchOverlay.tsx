...
export default function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  const highlightMatch = (text: string, q: string) => {
    ...
    return parts.map((part, i) => regex.test(part) ? <mark key={i} className="bg-[#8B5CF6]/30 text-white rounded px-0.5">{part}</mark> : part);
  };

  const getIcon = (type: SearchResult['type']) => {
    switch (type) { case 'movie': return <Film className="w-4 h-4 text-[#8B5CF6]" strokeWidth={1.5} />; case 'review': return <MessageSquare className="w-4 h-4 text-[#8B5CF6]" strokeWidth={1.5} />; case 'news': return <Newspaper className="w-4 h-4 text-[#22c55e]" strokeWidth={1.5} />; }
  };
      ...
      <div className="w-full max-w-2xl">
        <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-2xl shadow-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-[#1e1e28]">
            <Search className="w-5 h-5 text-[#8B5CF6] flex-shrink-0" strokeWidth={1.5} />
            ...
            {isSearching && <Loader2 className="w-4 h-4 animate-spin text-[#8B5CF6]" strokeWidth={1.5} />}
            {query && !isSearching && (
                ...
                <>
                  ...
                  <div className="px-4 py-3 border-t border-[#1e1e28]">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-[#8B5CF6] hover:bg-[#8B5CF6]/10 transition-colors text-sm font-medium"
                    >
