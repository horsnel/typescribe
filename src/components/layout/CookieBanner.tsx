...
export default function CookieBanner() {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-50 p-4 transition-all duration-300 ${animateIn ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
      <div className="max-w-4xl mx-auto bg-[#12121a] border border-[#2a2a35] rounded-2xl shadow-2xl overflow-hidden">
        {!showCustomize ? (
          <div className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                  ...
                  <Button onClick={() => setShowCustomize(true)} variant="outline" className="border-[#2a2a35] bg-transparent text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] text-sm">Customize</Button>
                  <Button onClick={handleReject} variant="ghost" className="text-[#6b6b7b] hover:text-white text-sm">Reject Non-Essential</Button>
                </div>
              </div>
              <button onClick={handleReject} className="p-1 text-[#6b6b7b] hover:text-white transition-colors flex-shrink-0" aria-label="Close"><X className="w-4 h-4" /></button>
            </div>
          </div>
        ) : (
          <div className="p-6">
            ...
            <div className="space-y-4 mb-6">
              <div className="flex items-center gap-4 p-3 bg-[#0a0a0f] rounded-xl border border-[#2a2a35]">
                <Shield className="w-5 h-5 text-[#22c55e] flex-shrink-0" />
                <div className="flex-1"><p className="text-sm font-medium text-white">Essential Cookies</p><p className="text-xs text-[#6b6b7b]">Required for the site to function. Cannot be disabled.</p></div>
                <span className="text-xs text-[#22c55e] font-medium">Always Active</span>
              </div>
              <label className="flex items-center gap-4 p-3 bg-[#0a0a0f] rounded-xl border border-[#2a2a35] cursor-pointer">
                <BarChart3 className="w-5 h-5 text-[#a0a0b0] flex-shrink-0" />
                <div className="flex-1"><p className="text-sm font-medium text-white">Analytics Cookies</p><p className="text-xs text-[#6b6b7b]">Help us understand how visitors interact with our site.</p></div>
              ...
              <label className="flex items-center gap-4 p-3 bg-[#0a0a0f] rounded-xl border border-[#2a2a35] cursor-pointer">
                <Settings className="w-5 h-5 text-[#a0a0b0] flex-shrink-0" />
                <div className="flex-1"><p className="text-sm font-medium text-white">Preference Cookies</p><p className="text-xs text-[#6b6b7b]">Remember your settings and personalization choices.</p></div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Button onClick={handleAccept} variant="outline" className="border-[#2a2a35] text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] text-sm">Accept All</Button>
              <button onClick={() => setShowCustomize(false)} className="text-sm text-[#6b6b7b] hover:text-white ml-2">Back</button>
            </div>
          </div>
        )}
      </div>
