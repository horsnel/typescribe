    ...
    <div className="min-h-screen bg-[#050507] pt-20 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-8">
        <div className="flex items-center gap-3 mb-4">
          <Bookmark className="w-6 h-6 text-[#8B5CF6]" strokeWidth={1.5} />
        ...
        {watchlistMovies.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-4 text-center">
              <Calendar className="w-5 h-5 text-[#8B5CF6] mx-auto mb-1" strokeWidth={1.5} />
        ...
        {watchlistMovies.length > 0 && (
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            <div className="flex gap-2 items-center">
              ...
              {genreNames.length > 1 && (
                <Button variant="outline" onClick={() => setFiltersOpen(!filtersOpen)} className={`border-[#1e1e28] bg-transparent text-[#9ca3af] hover:bg-[#111118] hover:text-white gap-1 text-xs ${filtersOpen ? 'border-[#8B5CF6]' : ''}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {genreNames.map((name) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                    selectedGenres.includes(name) ? 'bg-[#8B5CF6] text-white' : 'bg-[#050507] border border-[#1e1e28] text-[#9ca3af] hover:text-white hover:border-[#3a3a45]'
                  }`}
                >
                  {name}
                </button>
        ...
        {watchlistMovies.length > 0 ? (
          filtered.length > 0 ? (
            ...
            <div className="text-center py-16">
              ...
              <button onClick={() => setSelectedGenres([])} className="text-[#8B5CF6] hover:underline">Clear genre filter</button>
            </div>
          )
        ) : (
          <div className="text-center py-24">
            ...
            <Link href="/browse" className="text-[#8B5CF6] hover:underline">Browse Movies</Link>
          </div>
        )}
      </div>
    </div>
