...
export default function TopRatedPage() {
  ...
  const rankBadge = (rank: number) => {
    if (rank === 1) return <Crown className="w-6 h-6 text-[#8B5CF6]" strokeWidth={1.5} />;
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <Trophy className="w-8 h-8 text-[#8B5CF6]" strokeWidth={1.5} />
                ...
                {filtered.slice(0, 3).map(({ rank, movie, consensus }) => (
                  <Link
                    ...
                    className={`group relative bg-gradient-to-br ${
                      rank === 1 ? 'from-[#8B5CF6]/10 to-[#8B5CF6]/5 border-[#8B5CF6]/30' :
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-lg font-bold text-[#8B5CF6]">{movie.vote_average.toFixed(1)}</span>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-base font-bold text-white group-hover:text-[#8B5CF6] transition-colors leading-snug">{movie.title}</h3>
              ...
              <div className="space-y-3">
                {visible.map(({ rank, movie, consensus }) => (
                  <Link key={movie.id} href={`/movie/${movie.slug}`} className="flex items-center gap-6 bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 hover:border-[#3a3a45] hover:shadow-lg transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-lg font-bold text-white group-hover:text-[#8B5CF6] transition-colors">{movie.title}</h2>
