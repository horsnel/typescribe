    ...
    <div className="min-h-screen bg-[#050507] pt-8 pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-12 py-8">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="relative group">
            ...
            <Dialog open={avatarDialogOpen} onOpenChange={setAvatarDialogOpen}>
              <DialogTrigger asChild>
                <button
                  className="absolute bottom-1 right-1 w-9 h-9 bg-[#0c0c10] border border-[#1e1e28] rounded-full flex items-center justify-center text-[#9ca3af] hover:text-white hover:border-[#e50914] transition-all shadow-lg"
                  ...
                  {PRESET_AVATARS.map((av) => (
                    <button
                      ...
                      className={`relative rounded-xl p-1 transition-all hover:scale-105 ${
                        user.avatar === av.url ? 'ring-2 ring-[#e50914] bg-[#e50914]/10' : 'bg-[#050507] border border-[#1e1e28] hover:border-[#3a3a45]'
                      }`}
                    >
          ...
          <p className="text-sm text-[#9ca3af] mt-2 leading-relaxed text-center max-w-md">{user.bio || 'No bio yet. Click edit to tell us about yourself and your movie tastes.'}</p>
          <div className="flex items-center gap-4 mt-3 flex-wrap justify-center">
            {user.favorite_genres && user.favorite_genres.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[#9ca3af]">{user.favorite_genres.join(', ')}</span>
              </div>
            )}
            <div className="flex items-center gap-1">
              <Calendar className="w-4 h-4 text-[#6b7280]" />
              <span className="text-xs text-[#6b7280]">Member since {new Date(user.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span>
            </div>
            {user.public_profile && (
              <div className="flex items-center gap-1">
          ...
          <Link href="/profile/edit" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#111118] border border-[#1e1e28] rounded-lg text-sm text-white hover:border-[#3a3a45] transition-colors">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 text-center hover:border-[#3a3a45] transition-colors">
          ...
          <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 text-center hover:border-[#3a3a45] transition-colors">
            ...
            <div className="text-xs text-[#6b7280]">Watchlist</div>
          </div>
          <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 text-center hover:border-[#3a3a45] transition-colors">
            <TrendingUp className="w-6 h-6 text-green-400 mx-auto mb-2" />
            <div className="text-2xl font-bold text-white">24</div>
            <div className="text-xs text-[#6b7280]">Helpful Votes</div>
          </div>
        </div>

        {/* Quick Links */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <Link href="/watchlist" className="flex items-center gap-3 bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 hover:border-[#3a3a45] transition-colors group">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-[#6b7280] mt-0.5">{watchlistCount} movies saved</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#6b7280] group-hover:text-white transition-colors" />
          </Link>
          <Link href="/my-reviews" className="flex items-center gap-3 bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 hover:border-[#3a3a45] transition-colors group">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-[#6b7280] mt-0.5">{userReviewList.length} reviews written</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#6b7280] group-hover:text-white transition-colors" />
          </Link>
          <Link href="/settings" className="flex items-center gap-3 bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-5 hover:border-[#3a3a45] transition-colors group">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-[#6b7280] mt-0.5">Notifications & privacy</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#6b7280] group-hover:text-white transition-colors" />
          </Link>
        </div>
        ...
        <div className="flex items-center border-b border-[#1e1e28] mb-6">
          <button
            ...
            className={`pb-3 px-4 text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'activity' ? 'text-white border-b-2 border-[#e50914]' : 'text-[#6b7280] hover:text-[#9ca3af]'}`}
          >
            <Clock className="w-4 h-4" /> Recent Activity
          </button>
          <button
            onClick={() => setActiveTab('stats')}
            className={`pb-3 px-4 text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'stats' ? 'text-white border-b-2 border-[#e50914]' : 'text-[#6b7280] hover:text-[#9ca3af]'}`}
          >
          ...
          <>
            {activityItems.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-[#6b7280]">Start reviewing movies to see your activity here</p>
              </div>
            )}
          </>
        )}
