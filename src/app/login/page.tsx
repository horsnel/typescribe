        ...
        {showForgotPassword ? (
          ...
          <div className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-3 mb-2">
              <KeyRound className="w-5 h-5 text-[#8B5CF6]" strokeWidth={1.5} />
            ...
            {forgotSent ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => { setShowForgotPassword(false); setForgotSent(false); }} className="text-sm text-[#8B5CF6] hover:underline mt-4">Back to Sign In</button>
              </div>
            ) : (
              <>
                ...
                <div>
                  ...
                  <input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} required className="w-full bg-[#050507] border border-[#1e1e28] rounded-lg py-2.5 px-3 text-white placeholder:text-[#6b7280] focus:outline-none focus:border-[#8B5CF6]" placeholder="you@example.com" />
                </div>
                <Button onClick={handleForgotPassword} className="w-full bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-medium">Send Reset Link</Button>
            ...
            <form onSubmit={handleSubmit} className="bg-[#0c0c10] border border-[#1e1e28] rounded-xl p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2">
                  <input type="checkbox" className="rounded border-[#1e1e28] bg-[#050507] text-[#8B5CF6] focus:ring-[#8B5CF6]" />
                  <span className="text-xs text-[#6b7280]">Remember me</span>
                </label>
                <button type="button" onClick={() => setShowForgotPassword(true)} className="text-xs text-[#8B5CF6] hover:underline">
                  Forgot password?
                </button>
              </div>

              <Button type="submit" disabled={loading} className="w-full bg-[#8B5CF6] hover:bg-[#7C3AED] text-white font-medium">
                {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" strokeWidth={1.5} /> : 'Sign In'}
              </Button>

              <p className="text-center text-sm text-[#6b7280]">
                Don&apos;t have an account?{' '}
                <Link href="/signup" className="text-[#8B5CF6] hover:underline">Sign Up</Link>
              </p>
            </form>
          </>
        )}
