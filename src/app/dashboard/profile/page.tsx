  ...
  return (
    <DashboardSidebar>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/dashboard/settings"><Button variant="outline" className="border-[#2a2a35] text-white hover:bg-[#1a1a25] gap-2"><Edit3 className="w-4 h-4" />Edit Settings</Button></Link>
      </div>
      ...
      <div className="bg-[#12121a] border border-[#2a2a35] rounded-xl p-6 mb-6">
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-[#6b6b7b] mt-1">{user?.email}</p>
          <div className="flex items-center gap-2 text-xs text-[#6b6b7b] mt-2">
            <Calendar className="w-3.5 h-3.5" />
            <span>Member since {memberSince}</span>
          </div>
          {user?.bio && (
            <p className="text-sm text-[#a0a0b0] mt-3 text-center max-w-md">{user.bio}</p>
          )}
          {user?.favorite_genres && user.favorite_genres.length > 0 && (
            <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
              {user.favorite_genres.map(genre => (
                <span key={genre} className="text-xs text-[#a0a0b0] bg-[#1a1a25] border border-[#2a2a35] px-2.5 py-1 rounded-full">{genre}</span>
              ))}
            </div>
          )}
        </div>
      ...
      {showAvatarPicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setShowAvatarPicker(false)}>
          <div className="bg-[#12121a] border border-[#2a2a35] rounded-xl p-6 max-w-md w-full mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">Choose Avatar</h3>
              <button onClick={() => setShowAvatarPicker(false)} className="text-[#6b6b7b] hover:text-white">✕</button>
            </div>
            <div className="grid grid-cols-4 gap-3 mb-4">
              {AVATAR_PRESETS.map(url => (
                <button
                  ...
                  className={`w-16 h-16 rounded-full overflow-hidden border-2 transition-all hover:border-[#e50914] ${user?.avatar === url ? 'border-[#e50914] ring-2 ring-[#e50914]/30' : 'border-[#2a2a35]'}`}
                >
                  <img src={url} alt="Avatar" className="w-full h-full object-cover" />
                </button>
              ))}
            </div>
            <button onClick={handleRemoveAvatar} className="text-sm text-[#6b6b7b] hover:text-white transition-colors">Remove avatar (use initials)</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-[#12121a] border border-[#2a2a35] rounded-xl p-6">
        ...
        <div className="space-y-2">
          <Link href="/dashboard/reviews" className="flex items-center gap-3 px-3 py-2.5 text-sm text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] rounded-lg transition-colors">
            <Star className="w-4 h-4" /> My Reviews
          </Link>
          <Link href="/dashboard/watchlist" className="flex items-center gap-3 px-3 py-2.5 text-sm text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] rounded-lg transition-colors">
            <Bookmark className="w-4 h-4" /> My Watchlist
          </Link>
          <Link href="/dashboard/communities" className="flex items-center gap-3 px-3 py-2.5 text-sm text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] rounded-lg transition-colors">
            <Users className="w-4 h-4" /> My Communities
          </Link>
          <Link href="/dashboard/settings" className="flex items-center gap-3 px-3 py-2.5 text-sm text-[#a0a0b0] hover:text-white hover:bg-[#1a1a25] rounded-lg transition-colors">
