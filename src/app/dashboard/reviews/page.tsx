    ...
    <DashboardSidebar>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          <select value={filter} onChange={e => setFilter(e.target.value)} className="bg-[#12121a] border border-[#2a2a35] rounded-lg py-1.5 px-3 text-sm text-[#a0a0b0] focus:outline-none focus:border-[#e50914]">
            <option value="all">All Types</option>
            <option value="movie">Movies</option>
            <option value="tv">Series</option>
            <option value="anime">Anime</option>
          </select>
          <select value={sort} onChange={e => setSort(e.target.value)} className="bg-[#12121a] border border-[#2a2a35] rounded-lg py-1.5 px-3 text-sm text-[#a0a0b0] focus:outline-none focus:border-[#e50914]">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      {review.movie_slug ? (
                        <Link href={`/movie/${review.movie_slug}`} className="text-base font-semibold text-white hover:text-[#e50914] transition-colors truncate">
                    ...
                    {isSpoiler && !isRevealed ? (
                      <div className="relative">
                        <p className="text-sm text-[#a0a0b0] blur-sm select-none leading-relaxed">{review.text}</p>
                        <button
                          onClick={() => toggleSpoiler(review.id)}
                          className="absolute inset-0 flex items-center justify-center bg-[#12121a]/80 rounded-lg hover:bg-[#12121a]/60 transition-colors"
                        >
                      ...
                      <p className="text-sm text-[#a0a0b0] leading-relaxed line-clamp-3">{review.text}</p>
                    )}
                  </div>
                  {/* Actions */}
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => handleDelete(review.id)} className="p-2 text-[#6b6b7b] hover:text-red-400 transition-colors rounded-lg hover:bg-[#1a1a25]">
