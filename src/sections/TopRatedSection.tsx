...
const rankColors: Record<number, string> = { 1: 'text-[#8B5CF6]', 2: 'text-[#c0c0c0]', 3: 'text-[#cd7f32]' };
const rankBgColors: Record<number, string> = { 1: 'bg-[#8B5CF6]/20', 2: 'bg-[#c0c0c0]/20', 3: 'bg-[#cd7f32]/20' };
        ...
        {loading ? (
          ...
          <div className="card-grid grid grid-cols-1 md:grid-cols-3 gap-6">
            {displayMovies.map((movie, i) => {
              const rank = i + 1;
              return (
                <Link key={movie.id} href={`/movie/${movie.slug}`} className="card-reveal bg-[#0c0c10] border border-white/[0.06] rounded-xl overflow-hidden hover:border-[#8B5CF6]/30 hover:shadow-xl transition-all group">
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3"><span className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${rankBgColors[rank]} backdrop-blur-sm ${rankColors[rank]} text-2xl font-extrabold border-2 ${rank === 1 ? 'border-[#8B5CF6]' : rank === 2 ? 'border-[#c0c0c0]' : 'border-[#cd7f32]'}`} style={{ textShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>{rank}</span></div>
                  </div>
                  <div className="p-5">
                    <h3 className="text-lg font-bold text-white mb-1 group-hover:text-[#8B5CF6] transition-colors">{movie.title}</h3>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="flex items-center gap-1"><Star className="w-4 h-4 text-[#8B5CF6] fill-[#8B5CF6]" strokeWidth={1.5} /><span className="text-sm font-semibold text-[#8B5CF6]">{movie.vote_average.toFixed(1)}</span></div>
