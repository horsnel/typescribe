...
const iconConfig: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; bg: string }> = {
  Sword:  { icon: Sword,  color: 'text-red-500',     bg: 'bg-red-500/10' },
  Laugh:  { icon: Laugh,  color: 'text-orange-500',   bg: 'bg-orange-500/10' },
  Drama:  { icon: Drama,  color: 'text-purple-500',   bg: 'bg-purple-500/10' },
  Ghost:  { icon: Ghost,  color: 'text-red-400',      bg: 'bg-red-400/10' },
  Eye:    { icon: Eye,    color: 'text-blue-500',     bg: 'bg-blue-500/10' },
  Rocket: { icon: Rocket, color: 'text-indigo-500',   bg: 'bg-indigo-500/10' },
  Heart:  { icon: Heart,  color: 'text-pink-500',     bg: 'bg-pink-500/10' },
  Film:   { icon: Film,   color: 'text-gray-400',     bg: 'bg-gray-400/10' },
};
...
function GenreCard({ genre }: { genre: { id: string; name: string; icon: string; count: number } }) {
  ...
  return (
    <Link
      ...
      className="group relative flex items-center gap-4 p-4 rounded-2xl bg-[#0c0c10] border border-white/[0.06] hover:border-[#e50914]/30 hover:bg-[#111118] transition-all duration-300 cursor-pointer overflow-hidden"
    >
      {/* Sheen overlay */}
      ...
      >
        <IconComp className={`w-5 h-5 ${config.color}`} strokeWidth={2.5} />
      </div>
      ...
      {/* Hover arrow */}
      <ChevronRight size={16} className="ml-auto text-[#6b7280] opacity-0 group-hover:opacity-100 transition-opacity relative z-10" strokeWidth={2.5} />
    </Link>
  );
}
